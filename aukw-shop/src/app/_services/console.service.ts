...
export class ConsoleService {
  ...
  sendAllocationsToConsole(empAllocations: EmployeeAllocation[]) {
    empAllocations.forEach((employeeAllocation) => {
      const msg: string =
        `${employeeAllocation.name} : ${employeeAllocation.percentage}% ` +
  ...
  sendPayslipsToConsole(payslips: IrisPayslip[]) {
    if (!payslips || payslips.length == 0) {
      this.consoleMessageSubject.next('<none>');
    }
    payslips.forEach((payslip) => {
      const msg: string =
        ...
        `: MissingAllocs:${payslip.allocationsMissingFromQBO} : JournalInQBO:${payslip.qbFlags.employeeJournal}` +
  ...
  sendLineItemDetailToConsole(lines: LineItemDetail[]) {
    ...
    lines.forEach((line) => {
      const msg: string =
        `${line.name} : Salary:£${line.amount.toFixed(2)} ` +
