...
export const employeesResolver: ResolveFn<EmployeeName[]> = (route: ActivatedRouteSnapshot, state: RouterStateSnapshot) => {
  const qbEmployeeService = inject(QBEmployeeService);
  //const userId = route.paramMap.get('id')!;
  return qbEmployeeService.getAll(environment.qboCharityRealmID);
};

export const classesResolver: ResolveFn<QBClass[]> = (route: ActivatedRouteSnapshot, state: RouterStateSnapshot) => {
