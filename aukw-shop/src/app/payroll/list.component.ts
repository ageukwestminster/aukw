...
import { AuthenticationService, QBPayrollService, QBRealmService } from '@app/_services';
...
export class PayslipListComponent implements OnInit{
  ...
  ngOnInit() {    
    ...
    this.qbRealmService.getAll(this.user.id)
      .pipe(
        concatMap((realms:QBRealm[]) => {
          ...
          return this.qbPayrollService.getAllocations(this.charityRealm.realmid!);
        })
      )
  ...
  employerNI() {
    ...
    const employerNIArray = this.payslips.map(
      (p:IrisPayslip) => {
        return {
