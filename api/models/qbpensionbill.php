...
class QuickbooksPensionBill extends QuickbooksBill{
  ...
  public function create():array|false {
    ...
    $this->bill_line($bill['Line'],"Monthly total of salary sacrifices.",
        $this->salarySacrificeTotal,QBO::ADMIN_CLASS,
        QBO::SALARY_SACRIFICE_ACCOUNT,QBO::NOVAT_TAX_CODE);

    $this->bill_line($bill['Line'],"Monthly total of employee pension contributions.",
        $this->employeePensContribTotal,QBO::ADMIN_CLASS,
        QBO::EMPLOYEE_PENSION_CONTRIB_ACCOUNT,QBO::NOVAT_TAX_CODE);

    foreach($this->pensionCosts as $pensionAllocation) {
      //&$line_array, $description, $amount, $class, $account, $taxcoderef)
      $this->bill_line($bill['Line'], $pensionAllocation->name, 
        $pensionAllocation->amount, $pensionAllocation->class,
