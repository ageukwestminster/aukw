...
class UserToken
{
     ...
     *
     */
    public function store(int $iduser, string $accessTokenJti, string $refreshTokenJti, bool $status, $expiresAt)
    {
        $query = "INSERT INTO
                    " . $this->table_name . "
                    SET 
    ...
    public function getAccessTokenStatus($iduser, $key)
    {
    ...
    public function getRefreshTokenStatus($iduser, $key)
    {
        $query = "SELECT status FROM
                    " . $this->table_name . "
                 WHERE
